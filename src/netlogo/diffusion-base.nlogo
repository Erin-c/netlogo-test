...
to setup
  ...
  ;; adjusted the limit for the whole world (50->90)
  set limit 90
  create-erasers 1 [set hidden? true set size 3 set color white]
  ;; import the bitmap
  ask patches [set pcolor gray]
  ask patches with [pycor < 90 and pycor > -90 and pxcor > -90 and pxcor < 90 ][set pcolor white]
  ;;
...
;;  create-particles 500 [     ;;replacing with blocks
;;    setup-particle
;;    random-position
;;  ]
  setup-blocks
...
to random-position ;; particle procedure
  ...
  ;; changing this so particles occupy the whole world
...
;;  mouseme
  ...
  mouseme
  ;; moved those functions to blocks  
...
to bounce  ;; particle procedure  ;; get the coordinates of the patch we will be on if we go forward 1
  if [pcolor ] of patch-here != white [die ask one-of particles with [color = particle-color][hatch 1 [setup-particle] ]]
...
;;  ifelse [pcolor] of new-patch = white and new-py < limit [
  ;; removing the limit constraint (related to the bitmpa image)
...
to factor-gravity  ;; turtle procedure
  let gravity 0
  ifelse color = particle-color [set gravity 0.01 ][set gravity .01 ]
  let vx (dx * speed)
...
to move  ;; particle procedure
  ...
  setxy xcorr ycorr
  if gravity-check [ ;;how to test if it works?
    ;; ask particles [set color red] ;; testing the code
    factor-gravity
  ]
end
...
;  if count other particles-here = 1 ;; modified to be realistic, was = 1
...
; --- Start Blockly NetLogo Code ---

; --- Blockly NetLogo Code End ---
@#$#@#$#@
GRAPHICS-WINDOW
210
10
...
275
left temperature
mean [speed] of particles with [xcor < 0]
1
1
11
...
43
clear water
ask particles with [color = blue][die]
NIL
1
T
OBSERVER
